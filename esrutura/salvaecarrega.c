...
void CarregaDados(){2
    ...
    while(1){
        ...
        if (feof(fp))
            break;
        RemoveBarraN(str);
        strcpy(discente[qDiscente].matricula, str);
        fgets(str, 101, fp);
        RemoveBarraN(str);
        strcpy(discente[qDiscente].nome, str);
        fscanf(fp, "%f %f %f%*c", &discente[qDiscente].nota[1], &discente[qDiscente].nota[2], &discente[qDiscente].nota[3]);
        qDiscente++;
    }
...
void SalvaDados(int quantAlunos){
    ...
    for(i = 0; i < quantAlunos; i++){
        fprintf(fp, "%s\n%s\n%f %f %f\n", discente[i].matricula, discente[i].nome, discente[i].nota[1], discente[i].nota[2],discente[i].nota[3]);
    }
